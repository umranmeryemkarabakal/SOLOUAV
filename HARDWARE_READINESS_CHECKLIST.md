...
| B0 | **Kartta iniş/görev yolu YOK** | 🔴 **AÇIK** (Adım 152) — modül `z_sp`'yi `tiltrotor_indi_setpoint`'ten alır; iniş DIZISI `sitl/run_mission_test.py`'de, yani PC tarafında. O betik hedefi `px4-mc_indi_tiltrotor test_sp` **POSIX kabuk istemcisiyle** gönderir ve bu yol YALNIZCA SITL'de vardır (`indi_sitl_common.py` başlığı: "external MAVLink clients cannot publish arbitrary uORB topics"). Kartta karşılığı yok; `flight_mode_manager`/`mc_pos_control` bilerek durdurulmuş, yani PX4'ün land modu da yok. Kalan tek yol manuel gaz kolu (`_man_z_sp`) → RC yoluna bağlı → madde H8, sınanmadı. **Kart bugün flash edilirse otonom iniş olmaz.** |
