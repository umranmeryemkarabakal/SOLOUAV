...
| B0 | Kartta iniş/görev yolu | 🟢 **KAPANDI** (Adım 154) — **tam görev artık modülde**. `missionSequencer()` + `MissionState` (CLIMB→HOVER→FWD→CRUISE→FW→FW_CRUISE→BACK→SETTLE→LAND→DONE) tek bir `mission_enable` bayrağıyla sürülür; ft/bt/fw/land bayraklarını ve z_sp'yi **dizici üretir**. Geçişler olay tabanlı (ft_state==CRUISE, fw_state==ACTIVE, bt_state==HANDOFF, land_state==TOUCHDOWN); yalnızca seyir süreleri zamanlı. Zaman aşımında **geri geçişe düşer** — her arızada araç hover'a döner ve iner. SITL'de doğrulandı: 10 evrenin 10'u, 157 s, tırmanışta v_h 0,28 m/s, sabit kanatta tilt 90°/14,4 m/s, temas agl 1,21 m. Test: `sitl/run_autonomous_mission.py` (yalnızca bayrağı kaldırır ve izler). Modül **DISARM ETMEZ** — DONE yalnızca bildirir. ⚠ Kartta bayrağı kaldırmak hâlâ RC yoluna bağlı (H8). |
